...
int IocpPendingCounter::get(PVOID caller)
{
    ...
    if (iter == items_.end())
        ...
        return max(0, iter->second.sendCount + iter->second.recvCount);
}
...
{
    ThrowException(formatString(SEM_IOCP_ERROR, GetLastError()).c_str());
}
...
IocpOverlappedData* IocpObject::createOverlappedData(IOCP_TASK_TYPE taskType,
    ...
    const IocpCallback& callback, PVOID caller, const Context& context)
{
    ASSERT_X(buffer != NULL);
	ASSERT_X(size >= 0);
	ASSERT_X(offset >= 0);
	ASSERT_X(offset < size);
